...
func TestSign(t *testing.T) {
	...
	isok(t, sig.VerifyKey(key.PublicKey, digest, crypto.SHA256))
	isok(t, rsa.VerifyPKCS1v15(&key.PublicKey, crypto.SHA256, digest, sig.Signature))
}

func TestSignPool(t *testing.T) {
	pool := ima.NewKeyPool()
	...
	usedKey, err := sig.Verify(ima.VerifyOptions{
		Keys:   pool,
		Digest: digest,
		Hash:   crypto.SHA256,
	})
	assert(t, err == ima.UnknownSigner)
